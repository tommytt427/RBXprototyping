...
function tower.FindTarget(newTower, mode)
	...
	if not map or not map:FindFirstChild("Waypoints") then
		warn("Map or Waypoints not found")
		return nil
	end
	
	for _, target in ipairs(workspace.Mobs:GetChildren()) do
		if target:FindFirstChild("HumanoidRootPart") and IsTargetWithinHitbox(target, newTower) then
	...
	if bestTarget then
		print("Nearest target: ", bestTarget.Name)
	else
...
function tower.Attack(newTower, player)
	while newTower and newTower.Parent do
		...
		local currentTarget = tower.FindTarget(newTower, config.TargetMode.Value)

		if currentTarget and IsTargetWithinHitbox(currentTarget, newTower) and currentTarget:FindFirstChildOfClass("Humanoid") and currentTarget.Humanoid.Health > 0 then
...
function tower.ManageBlockedMobs(newTower)
	while newTower and newTower.Parent do
		local config = newTower.Config
		local blockedCount = #blockedMobs[newTower]

		-- Remove any destroyed mobs from the blocked list
		for i = #blockedMobs[newTower], 1, -1 do
			if not blockedMobs[newTower][i].Parent then
				table.remove(blockedMobs[newTower], i)
			end
		end

		-- If blocked count is less than Block value, look for mobs to block
		if blockedCount < config.Block.Value then
			for _, mob in ipairs(workspace.Mobs:GetChildren()) do
				if mob:FindFirstChild("HumanoidRootPart") and not mob:FindFirstChild("Blocked") then
					local distance = (newTower.HumanoidRootPart.Position - mob.HumanoidRootPart.Position).Magnitude
					if distance <= 5 then  -- Adjust this value as needed
						local blockedValue = Instance.new("BoolValue")
						blockedValue.Name = "Blocked"
						blockedValue.Value = true
						blockedValue.Parent = mob
						table.insert(blockedMobs[newTower], mob)
						break
					end
				end
			end
...
function tower.Spawn(player, name, cframe, previous)
	...
	if allowedToSpawn then
		...
		--Add Block NumberValue to Config
		local blockValue = Instance.new("NumberValue")
		blockValue.Name = "Block"
		blockValue.Value = 2
		blockValue.Parent = newTower.Config
		...
		-- initalizing blocked mobs table for the tower
		blockedMobs[newTower] = {}
		local height = (newTower.HumanoidRootPart.Size.Y / 2) + newTower.Humanoid.HipHeight
		...
		p.CastShadow = false
		p.Parent = newTower
		p.CFrame = newTower.HumanoidRootPart.CFrame * offset - Vector3.new(0, -0.637, 0)
		p.Size = Vector3.new(10.01, 10, 10.01)
		...
		coroutine.wrap(tower.ManageBlockedMobs)(newTower)
